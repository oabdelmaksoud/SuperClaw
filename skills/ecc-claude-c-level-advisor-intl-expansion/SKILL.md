...
## Purpose
Apply `c level advisor intl expansion` guidance from upstream references in an OpenClaw-native workflow.

## Trigger Conditions
- User request clearly matches `c level advisor intl expansion` capability.
...
- Requests unrelated to `c level advisor intl expansion` specialization.
