...
description: "OpenClaw bridge skill for c level advisor cro advisor. Use when tasks match this specialized anthropic skills capability and need OpenClaw-native execution with explicit verification."
---

# ecc-claude-c-level-advisor-cro-advisor

## Purpose
Apply `c level advisor cro advisor` guidance from upstream references in an OpenClaw-native workflow.

## Trigger Conditions
- User request clearly matches `c level advisor cro advisor` capability.
...
- Requests unrelated to `c level advisor cro advisor` specialization.
